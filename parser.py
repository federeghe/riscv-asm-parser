...
token_functions = token_function_full + pp.Suppress(pp.Char(':'))
...
# Pseudo OPS
token_pseudoops_arg_sym = pp.Combine(token_identifier + pp.Optional(pp.Char(',') + token_number + pp.Char(',') + token_number) + pp.Optional("," + token_identifier), adjacent=False, joinString=" ")
...
parsed = lines_all.parseFile("examples/sha1_64.s", parseAll=True)
print(parsed.dump())
